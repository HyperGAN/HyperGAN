...
2. [CPU collective groundwork](../docs/distributed.md) is implemented through `614b4410`: two-process Gloo fixtures cover differentiable global gathers/means, global unique prior IDs, MoG dense gradients, lazy b-cap derivatives and bounded missing-rank failures. Eight focused tests passed locally, and the sdist-built wheel passed all **89 installed-package tests** outside the checkout in 33.42 seconds on the recorded CPU runtime. Required CI remains the merge gate. This is not a DDP trainer or distributed recovery. Next add fixed-world-size two-process CPU execution and compare complete single-process versus distributed updates. Cover global batch/accumulation semantics, repeated prior IDs/global VICReg statistics, lazy penalties/double backward, synchronized EMA, named per-rank RNG/data state and whole-job failure/recovery. No GPU claim follows from Gloo fixtures.
...
Continue from current `develop`; PR #305 is the recovery integration point and must pass its required checks before merge. Do not restart branch or issue audits. Keep master historical until a separately qualified release; keep paid compute out of the next CPU core slice.
