...
## Next bounded checkpoint: complete CPU distributed updates

1. Build one fixed-world-size two-process CPU execution path around the established global-objective primitives. Own G/prior/auxiliary forward/reducers explicitly, alternate D/G updates safely, and compare complete parameters, Adam state and EMA with a controlled global reference. Cover global batch/accumulation, repeated IDs/global VICReg and exact lazy double-backward penalties; do not silently scale learning rates.
2. Add coordinated fixed-topology recovery: named per-rank RNG/data state, sampler ownership, rank agreement on complete checkpoints, failure timeouts and whole-job restart. A missing rank must never publish a successful global checkpoint. Existing Gloo failure fixtures only test collective failure, not job recovery.
...
5. The optional standalone viewer W2/W3 can now follow the implemented W1 read contract alongside distributed work. Keep it a separate process with the planned local access controls; ONNX/container deployment and release promotion remain later gates.
