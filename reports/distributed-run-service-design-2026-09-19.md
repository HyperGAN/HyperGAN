...
Date: 2026-09-19. **Distributed integration proposal.** Originally reviewed against develop `96222fc6` after PR #309. The first shared-controller/single-process extraction is now covered by the [lifecycle checkpoint](core-lifecycle-2026-09-19.md); the [CPU profile and preflight](core-preflight-2026-09-19.md) are also implemented. The [worker command and parent publication checkpoint](core-worker-service-2026-09-19.md) now supplies the supervised command protocol, parent-death cleanup and checkpoint prepare/commit split. The [internal replicated adapter](core-replicated-service-2026-09-19.md) now connects the controller and those primitives; bounded observers are implemented in the [observation checkpoint](core-bounded-observation-2026-09-19.md); public integration remains planned. The coordinator owns the status ledger and implementation sequencing. This report does not qualify GPU, NCCL, multi-node execution, image quality or a release.
...
The internal [replicated trainer](../src/hypergan/distributed_training.py) owns complete CPU Gloo updates, global data draws followed by rank slicing, replica agreement and readiness/poison state. The [checkpoint helper](../src/hypergan/distributed_checkpoints.py) preserves every rank's state, requires all-rank readiness before rank-zero publication and restores fixed topology strictly. The original [supervisor](../src/hypergan/cpu_workers.py) retains its finite blocking callback interface. The new [command service](../src/hypergan/cpu_worker_service.py) uses an independent broker to monitor and reap workers after coordinator death, including during a long operation. The [parent authority](../src/hypergan/distributed_commit.py) validates and publishes prepared state under the caller-owned run lock. The internal replicated adapter now supplies the shared-controller lifecycle; public distributed commands remain gated on bounded observation and the remaining acceptance cases.
...
| 3a | Internal supervised all-rank commands, checkpoint prepare/commit authority and parent-death cleanup | Implemented in the [worker service checkpoint](core-worker-service-2026-09-19.md); installed-package evidence and limits are recorded there |
| 3b | Implemented internal replicated adapter with fenced restore/attempt identity | See the [replicated service checkpoint](core-replicated-service-2026-09-19.md) for whole-job recovery/takeover and the remaining public gates |
...
The shared lifecycle, CPU profile/preflight, supervised commands, parent commit authority and internal replicated adapter with final artifacts are implemented. The [bounded observation checkpoint](core-bounded-observation-2026-09-19.md) implements isolated previews and bounded callback delivery. Remaining whole-job persistence faults and public CLI acceptance form the next usability gate. This does not complete distributed issue #186. Actual NCCL/two-GPU and real multi-node qualification, license resolution, selected image-experiment admission, dataset acquisition, deployment and the optional viewer remain the separate gates recorded in the [status ledger](resurrection-status.md) and [resurrection plan](resurrecting-hypergan-plan-2026-09-18.md).
