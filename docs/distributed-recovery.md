...
Compatibility checks include full recipe and schedule, actual runtime and numerical source identity, data/content/preprocessing/class-map identity, strategy, global/local batch and fixed world size. The publication module is included in distributed checkpoint source identity. This refactor therefore rejects older-source distributed checkpoints under the existing strict policy; there is no source-check bypass. Topology changes, new recipes and transfer initialization are not resume. Source identity covers the recorded modules; it is not proof that arbitrary hidden transitive Python dependencies or external state are reproducible.
