...
from hypergan.modules.modulated_conv2d import upfirdn2d
...
class StyleSwinLayer(hg.Layer):
    """
        ...
        ```json
          "style_swin_layer 3 upsample=true"
        ```
    """
    def __init__(self, component, args, options):
        super(StyleSwinLayer, self).__init__(component, args, options)

        dims = component.current_size.dims #TODO
        self.size = LayerShape(*dims)
        in_channel = component.current_size.dims[0]
        in_resolution = [component.current_size.dims[1], component.current_size.dims[2]]
        attn_drop_rate = 0.0
        depth = options.depth or 4
        drop_rate = 0.0
        mlp_ratio = options.mlp_ratio or 4
        num_heads = options.num_heads or 8
        qk_scale = None
        qkv_bias = True
        style_dim = options.style_dim or 512
        upsample = options.upsample or False
        window_size = options.window_size or (component.current_size.dims[1]//2)

        mlp_ratio = options.mlp_ratio or 4.0
        if upsample:
            upsample = BilinearUpsample
        self.transpose = options.transpose or False
        self.initial_transpose = options.initial_transpose or False

        self.layer = StyleBasicLayer(dim=in_channel,
                           ...
                           upsample=upsample,
                           use_checkpoint=False, style_dim=style_dim)
    def output_size(self):
        return self.size

    def forward(self, input, context):
        net = input
        if self.initial_transpose:
            B, C, H, W = net.shape
            net = net.permute(0, 2, 3, 1).contiguous().view(B, H*W, C)
        elif len(net.shape) == 4:
            # todo: rearrange hacked fixme
            B, C, H, W = net.shape #un-rearrange
            net = net.view(B, H*W, C)
        net = self.layer(net, context['w1'], context['w2'])
        B, HW, C = net.shape
        H = H*2
        W = W*2
        if self.transpose:
            net = net.reshape(B, H, W, C).permute(0, 3, 1, 2).contiguous()
        return net
