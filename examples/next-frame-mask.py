    ...
    def create(self):
        ...
        with tf.device(self.device):
            ...
            if config.style:
                ...
                encoder = self.create_component(config.encoder, input=t0 * mask, name='encoder')
                g_next_input = tf.concat([encoder.sample, random_like(encoder.sample)], axis=3)
                g_next = self.create_component(config.generator, input=g_next_input, name='g')
                for i, m in enumerate(masks):
                    self.g_frames.append(tf.slice(g_next.sample, [0,0,0,i*3], [-1,-1,-1,3]) * (1-m) + self.frames[i] * m)

            t1 = g_next.sample * (1-mask) + t0 * mask
            self.mask = mask
            self.next_frame_mask = next_frame_mask
            self.gx = g_next
            self.gy = g_next
            ...
            self.styleb = style
            self.stylea = style
            self.uniform_encoder = self.styleb

            self.uniform_sample = self.gx.sample
            ...
            if config.random_g:
                g_next_input = tf.concat([random_like(encoder.sample), random_like(encoder.sample)], axis=3)
                t2 = self.create_component(config.generator, input=g_next_input, name='g', reuse=True).sample
                t1 = g_next.sample
                stack = [t0, t1, t2]
                stacked = ops.concat(stack, axis=0)
                features = None#ops.concat([f0, f1, f2], axis=0)
            ...
            d_vars1 = d.variables()
            g_vars1 =  g_next.variables()  + encoder.variables()
            if config.style:
...
class VideoFrameSampler(BaseSampler):
    def __init__(self, gan, samples_per_row=8):
        ...
        self.next_frame_mask = gan.session.run(gan.next_frame_mask)
        BaseSampler.__init__(self, gan, samples_per_row)

    def _sample(self):
        gan = self.gan
        feed_dict = dict(zip(gan.frames, self.frames))
        feed_dict[gan.mask]=self.next_frame_mask
        sample = gan.session.run(gan.g_frames[-1], feed_dict=feed_dict)
...
class TrainingVideoFrameSampler(BaseSampler):
    ...
    def _sample(self):
        ...
        sess = gan.session
        feed_dict = {gan.mask: sess.run(gan.next_frame_mask)}
        
        frames = sess.run(gan.g_frames, feed_dict=feed_dict)
 
        return {
            'generator': np.vstack([np.hstack(frames)])
        }
