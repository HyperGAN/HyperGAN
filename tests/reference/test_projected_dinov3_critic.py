...
def test_projected_dino128_spatial_heads_and_frozen_projection(monkeypatch, tmp_path, batch_size):
    ...
    source = (Path(__file__).parents[2] /
              'research/startup_tuning/testbeds/transgan128-projected-dino/discriminator.hndl').read_text()
    ...
    torch.testing.assert_close(captured['normalized'], (x.detach() * .5 + .5 - mean) / std, rtol=2e-6, atol=2e-7)
    score.mean().backward()
