...
The [internal CUDA/NCCL profile](replicated-cuda.md) connects rank-owned devices, device-aware reductions, accumulated replay and coordinated recovery to the existing independent supervisor. Public execution-profile routing and bounded CLI output remain the next integration work. Actual two-host testing follows with a concrete cloud allocation.
