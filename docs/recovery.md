...
For process managers, `--progress-json` writes JSONL progress to stdout, followed by a `result` event containing the final manifest. Diagnostics remain on stderr. Without that flag, progress goes to stderr and the final manifest goes to stdout. CLI output is bounded best-effort delivery; the event journal is authoritative. The Python API starts no server; the CLI can launch the optional browser viewer.
