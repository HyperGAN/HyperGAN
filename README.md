...
Use `--preview-every N` for bounded periodic EMA previews, indexed by a short stable sample name (`g` generated, `x` real; rename the generated source with `--preview-name`) and kept for the whole run by default, so the viewer's per-sample history slider scrubs from the first sample to the latest (`--preview-keep N` opts into discarding older previews under disk pressure). `hypergan events RUN_DIR` reads reconnectable event pages, and `hypergan checkpoint RUN_DIR` requests a save at the next complete update boundary. These commands share the [run observation contract](docs/observation.md). Routine CLI progress defaults to every 100 updates; change it with `--progress-every N` or the persisted browser setting, independently of stored metrics.
