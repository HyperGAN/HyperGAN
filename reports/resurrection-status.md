...
The clean CPU foundation is implemented in PRs [#301](https://github.com/HyperGAN/HyperGAN/pull/301) and [#300](https://github.com/HyperGAN/HyperGAN/pull/300). Integration is complete when both PRs are merged with required checks passing. Resume by checking those live statuses and current develop, not by restarting the historical audit.
...
| F7 | Integrated local acceptance | Cleaned wheel/sdist build; 26 installed-package tests passed outside checkout; ten-command default/paired walkthrough passed. PR #300 required CI is the final integration gate |
...
2. Select and reproduce one small image recipe and its data/preprocessing contract. Keep custom components configurable; qualify exact profiles separately.
3. Implement full atomic training checkpoints: optimizers, counters, EMA/prior, named RNG streams, data/sampler position, schema and topology. Test uninterrupted versus interrupted/resumed runs. Current model.pt is inference-only and cannot resume training.
...
5. After these gates pass, test actual two-GPU NCCL locally. Only then prepare an explicitly bounded real two-node allocation and provider profile.
