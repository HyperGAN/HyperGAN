...
The image baseline is separate from the qualified synthetic numerical reference. The owner's 2026-09-19 selection is now the **SAGAN-style scratch generator plus pretrained ResNet18 discriminator, MoG16,384 and exact b-cap**, pinned by the [image plan](image-training-plan-2026-09-19.md). Its original 200k trajectory records FID50k12.5345. Preserve the actual source behavior, including GroupNorm, active attention, phase draws and encoder-only reconstruction; do not substitute vanilla DCGAN defaults. The earlier residual MoG1,024 candidate's FID50k19.483 at10k remains historical evidence in the [core recovery report](core-recovery-2026-09-18.md), not the next port target. Both source architectures live outside the installed package. Resolve provenance/licensing, then extract a reusable module or a narrow attributed port using public package primitives and complete HyperGAN recovery.
