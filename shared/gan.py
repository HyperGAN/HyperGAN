...
def discriminator_wide_resnet(config, x):
    ...
    def deconv_to_size(conv, start_resnet_at_height=64, end_resnet_at_height=16):
        result = conv
        height = result.get_shape()[2]
        i=0
        while(height > start_resnet_at_height):
            # downsample + identity
            result = conv2d(result, layers[0], name='d_expand'+str(i), k_w=3, k_h=3, d_h=2, d_w=2)
            result = batch_norm(config['batch_size'], name='d_expand_bn'+str(i))(result)
            result = activation(result)
            result = conv2d(result, layers[0], name='d_expand'+str(i)+'a', k_w=3, k_h=3, d_h=1, d_w=1)
            result = batch_norm(config['batch_size'], name='d_expand_bn'+str(i)+'a')(result)
            result = activation(result)
            height = result.get_shape()[2]
            i+=1
        i=0
        result = residual_block(result, activation, batch_size, 'widen', 'd_layers_wide')
        while(height > end_resnet_at_height):
            result = residual_block(result, activation, batch_size, 'conv', 'd_layers_conv_'+str(i))
            result = residual_block(result, activation, batch_size, 'identity', 'd_layers_identity_'+str(i))
            height = result.get_shape()[2]
            i+= 1
