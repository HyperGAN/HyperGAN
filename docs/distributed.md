...
`tests/reference/test_distributed_training.py` runs three bounded subprocess tests. Its parity fixture exercises all 12 Rp/RA/vanilla × logistic/hinge/Wasserstein/LSGAN combinations for three complete updates against `ReferenceTrainer`, comparing parameters, prior, both Adam states, base learning rates, EMA and metrics. It also tests a nonlinear critic's active and skipped lazy b-cap steps with distinct fixed real data. The pinned b-cap computes exact gradients at real and fake endpoints, without interpolation draws. Other cases cover global data slicing, Gaussian prior handling, absent gradient contributions, rank-local nonfinite input refusal, poisoned-state reuse and invalid accumulation/global-batch/config mismatches.

These float32 comparisons use explicit numerical tolerances, not bitwise equality. Reduction order changes floating-point cancellation. In particular, RA's additive critic-bias direction is mathematically null, and tiny residual gradients can be amplified by Adam's epsilon; the controlled RA fixture omits that bias. Likewise, the nonlinear Rp fixture uses distinct real and fake inputs instead of an identically zero adversarial discriminator gradient. Arbitrary architectures, data and custom objectives are not promoted to a qualified profile by these tests. Multi-GPU/NCCL, real clusters and a distributed run service remain separate gates. CPU accumulation now has independent complete-update, activation-memory and recovery tests described in the accumulation guide. Fixed-topology snapshot/restart evidence is documented in the recovery guide.
