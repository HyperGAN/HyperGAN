...
# Adversarial-only CIFAR TransGAN. Does not resume the reconstruction run.
exec "$RUNS_DIR/transgan-128-env/bin/python" -m hypergan train \
  "$WORKTREE/research/startup_tuning/testbeds/cifar-transgan32-adversarial/cifar-transgan.toml" \
  --run-dir "$RUNS_DIR/train-cifar-transgan-32-adversarial" \
