...
def g_resize_conv_create(config, gan, net):
    ...
    gs[-1] = tf.image.resize_images(gs[-1], [64,1], 1)


    gs[-1] = tf.reshape(gs[-1], [gan.config.batch_size, 64])
    return gs

def d_pyramid_create(gan, config, x, g, xs, gs, prefix='d_'):
    print("x,g",x,g)
    with tf.variable_scope("d_input_projection", reuse=False):
        x = linear(x, 64*64*3, scope=prefix+'input_projection')
        x = layer_norm_1(gan.config.batch_size, name=prefix+'_expand_bn_init')(x)
        x = config.activation(x)
        x = tf.reshape(x, [gan.config.batch_size, 64, 64, 3])
    with tf.variable_scope("d_input_projection", reuse=True):
        g = linear(g, 64*64*3, scope=prefix+'input_projection')
        g = layer_norm_1(gan.config.batch_size, name=prefix+'_expand_bn_init')(g)
        g = config.activation(g)
        g = tf.reshape(g, [gan.config.batch_size, 64, 64, 3])
...
with tf.device(args.device):
    ...
    with sess.as_default():
        print("Evaling name")
        xstr = ""
        name_v, asciix = sess.run([line, xorig])
        #asciix = asciix[0]
        asciix = np.round((asciix+1)*127.5)
        #asciix = np.array((asciix+1)*127.5)
        for item in asciix:
            try:
                xstr+=chr(item)
            except:
