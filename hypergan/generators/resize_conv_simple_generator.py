...
def create(config, gan, net, input=None):
    ...
    for j in range(config.hd_layers):
        name='g_layers_'+str(j)
        net = config.block(net, config, activation, batch_size, 'identity', 'g_layers_hd'+str(j), output_channels=int(net.get_shape()[3]), filter=3)
        print("[hd generator layer]", net)
    name= 'g_layers_end'
