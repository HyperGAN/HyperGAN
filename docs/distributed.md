...
`config` uses the normal resolved recipe schema. `training.batch_size` is the **global** batch size and must be divisible by the world size. `update(batch=..., latent_draw=(z, ids))` accepts explicit **local** shards for controlled comparisons. No learning rate is scaled automatically. A positive `accumulation_steps` factor must divide the local batch evenly. The [accumulation guide](accumulation.md) describes full-logit loss derivatives, microbatch replay, activation-memory bounds and custom-objective requirements; VICReg populations retain their full-batch semantics.

Each update computes D backward (including the exact lazy K3P critic penalty), averages complete D parameter gradients, steps D (whose recipe optimizer also runs the spike guard and advances the EMA critic identically on every rank), then computes and averages G/prior/auxiliary gradients, steps their shared optimizer, and advances EMA. The RpGAN loss pairs each real with the fake in its row, so rank-local logits suffice. Critic input and generator output noise are not supported here. Sampled-row VICReg uses the globally unique raw prior population once per update; standardized MoG still depends on the full replicated table. Custom scalar objectives are averaged across ranks: built-in elementwise mean MSE/L1 fits that contract, while custom global statistics require explicit collective logic and remain unqualified.
...
`tests/reference/test_distributed_training.py` runs three bounded subprocess tests. Its parity fixture runs the RpGAN loss with sampled and full prior-regularizer rows for three complete updates against `ReferenceTrainer`, comparing parameters, prior, both Adam states (including the EMA critic), base learning rates, EMA and metrics. It also tests a nonlinear critic's active and skipped lazy penalty steps with distinct fixed real data. The penalty computes exact gradients at real and fake endpoints, without interpolation draws. Other cases cover global data slicing, Gaussian prior handling, absent gradient contributions, rank-local nonfinite input refusal, poisoned-state reuse and invalid accumulation/global-batch/config mismatches.

These float32 comparisons use explicit numerical tolerances, not bitwise equality. Reduction order changes floating-point cancellation. The nonlinear Rp fixture uses distinct real and fake inputs instead of an identically zero adversarial discriminator gradient. Arbitrary architectures, data and custom objectives are not promoted to a qualified profile by these tests. Multi-GPU/NCCL, real clusters and a distributed run service remain separate gates. CPU accumulation now has independent complete-update, activation-memory and recovery tests described in the accumulation guide. Fixed-topology snapshot/restart evidence is documented in the recovery guide.
