...
def test_training_rng_replay_and_evaluation_identity(device):
    module = DiffAugment()
...
def test_augmentation_supports_generator_and_gradient_penalty_backward():
    x = torch.linspace(-1, 1, 3 * 8 * 8, dtype=torch.float64).reshape(1, 3, 8, 8).requires_grad_()
    state = torch.get_rng_state()

    def replay(value):
        torch.set_rng_state(state)
        return diff_augment(value)
