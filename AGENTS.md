...
The default test suite is fast and is always run: a plain `python -m pytest` deselects the `heavy` marker, which covers tests that start real subprocesses, multi-rank jobs, worker services or acceptance drivers. Run the heavy suite deliberately with `python -m pytest -m heavy` whenever a change touches training, checkpoint/resume, distributed or replicated execution, workers and services, or the CLI paths those tests drive, and before merging to develop. Heavy tests are deselected locally, never reported as passing skips. GitHub CI runs the heavy jobs only for pushes to master and PRs targeting master; develop runs the fast selection, viewer and reducer checks. The final CI gate requires heavy success on master and intentionally omitted heavy jobs elsewhere.
