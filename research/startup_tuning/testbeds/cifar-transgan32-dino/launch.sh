...
# Adversarial-only CIFAR TransGAN with the 32px DINOv3 critic.
exec "$RUNS_DIR/transgan-128-env/bin/python" -m hypergan train \
  "$WORKTREE/research/startup_tuning/testbeds/cifar-transgan32-dino/cifar-transgan.toml" \
  --run-dir "$RUNS_DIR/train-cifar-transgan-32-dinov3" \
