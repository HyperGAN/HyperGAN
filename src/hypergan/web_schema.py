...
def schemas():
    ...
    run = object_schema({'run_id': {'type': 'string'}, 'attempt_id': {'type': 'string'}, 'name': {'type': 'string'},
        ...
        'initialization_tuning': object_schema({
            ...
            'g_lr_factor': {'type': 'number', 'exclusiveMinimum': 0},
            'd_lr_factor': {'type': 'number', 'exclusiveMinimum': 0},
            'dynamics_outcome': {'enum': ['selected', 'kept_baseline', 'unresolved', 'skipped']},
            'selected_g_lr_factor': {'type': 'number', 'exclusiveMinimum': 0},
            'selected_d_lr_factor': {'type': 'number', 'exclusiveMinimum': 0},
            'dynamics_reason': {'oneOf': [{'type': 'string'}, {'type': 'null'}]},
            'g_lr_warmup': object_schema({'steps': SAFE_INTEGER,
                'start_g_lr': {'type': 'number', 'exclusiveMinimum': 0},
                'target_g_lr': {'type': 'number', 'exclusiveMinimum': 0}}),
            'message': {'type': 'string'}, 'selected_candidate': {'type': 'string'}}, ('status',)),
        'g_lr_warmup': object_schema({'steps': SAFE_INTEGER, 'completed_steps': SAFE_INTEGER,
            'progress': {'type': 'number', 'minimum': 0, 'maximum': 1},
            'status': {'enum': ['running', 'complete']},
            'g_lr': {'type': 'number', 'minimum': 0},
            'start_g_lr': {'type': 'number', 'exclusiveMinimum': 0},
            'target_g_lr': {'type': 'number', 'exclusiveMinimum': 0}}),
