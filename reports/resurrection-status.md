...
The following recovery integration on `resurrection/core-integration` adds complete CPU training checkpoints, explicit `resume`, cooperative stop limits, isolated inference/progress RNG, immutable attempt artifacts and versioned observed/durable progress. Implementation through `f49d52357594340bbb882aa93abf5e92eb929fbc` passed **79 installed-package tests**, using a new wheel built through the source distribution and tested outside the checkout. Runtime: Python 3.12.13, torch 2.14.0+cpu, ParticleGAN 0.5.0, NumPy 2.5.3, Pillow 12.3.0. Required PR CI must pass before merge.
...
Continue from current `develop` after the reviewed recovery PR merges. Do not restart branch or issue audits. Keep master historical until a separately qualified release; keep paid compute out of the next CPU core slice.
