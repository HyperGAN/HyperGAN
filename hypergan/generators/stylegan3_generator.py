...
class Generator(BaseGenerator):
    def __init__(self, gan, config, input=None):
        ...
        self.synthesis = SynthesisNetwork(w_dim=self.w_dim, img_resolution=self.img_resolution, img_channels=self.img_channels)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=self.z_dim, c_dim=self.c_dim, w_dim=self.w_dim, num_ws=self.num_ws)
    ...
    def forward(self, input):
        z = self.z_project(input)
        c = self.c_project(input)
        update_emas = self.update_emas
        ws = self.mapping(z, c, truncation_psi=self.truncation_psi, truncation_cutoff=self.truncation_cutoff, update_emas=update_emas)
        img = self.synthesis(ws, update_emas=self.update_emas)
        return img
