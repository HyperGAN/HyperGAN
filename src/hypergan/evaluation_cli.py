...
def add_evaluate_parser(commands):
    parser = commands.add_parser('evaluate', help='Evaluate a configured snapshot metric on an immutable EMA snapshot')
    parser.add_argument('run_dir', type=Path)
    parser.add_argument('--metric', required=True, help='enabled metrics.custom ID with mode=snapshot')
