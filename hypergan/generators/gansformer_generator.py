#adapted from https://github.com/dorarad/gansformer/blob/main/pytorch_version/training/networks.py
import numpy as np
import torch
import math
...
def get_positional_encoding(
    ...
    emb = crop_tensor(emb, crop_ratio)
    return emb, params
...
class SynthesisLayer(torch.nn.Module):
    def __init__(self,
        ...
        **transformer_kwargs            # Arguments for SynthesisLayer
        ...
        if local_noise:
            noise_shape = list(crop_tensor_shape((self.out_res, self.out_res), 
...
class SynthesisBlock(torch.nn.Module):
    def __init__(self,
        ...
        **layer_kwargs                      # Arguments for SynthesisLayer
        ...
        if self.stem:
            self.init_shape = crop_tensor_shape((self.res, self.res), layer_kwargs.get("crop_ratio"))
            if latent_stem:
...
class Generator(BaseGenerator):
    def __init__(self, gan, config, input=None, name=None):
        ...
        self.img_resolution = self.config.img_resolution or 128
        ...
        self.mapping = MappingNetwork(z_dim = self.z_dim, c_dim = self.c_dim, w_dim = self.w_dim, k = self.k, 
                num_broadcast = self.num_ws)

        self.z_project = FullyConnectedLayer(self.z_dim, self.w_dim)
        self.c_project = FullyConnectedLayer(self.z_dim, self.c_dim)
    ...
    def forward(self, input):
        ...
        return_ws = False
        subnet = None
        z = self.z_project(input).unsqueeze(1)
        c = self.c_project(input)
        ws = None
        return_tensor = True
        self.training = True
    ...
    def latent_parameters(self):
        return [self.z_project.weight, self.c_project.weight]
