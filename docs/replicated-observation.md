...
This first implementation is synchronous: there is at most one pending preview, and the next update waits for it. `service_policy.preview_timeout` defaults to 60 seconds and bounds isolated rendering; capture remains subject to the training command deadline. The training broker continues monitoring numerical worker health while the parent waits. Cleanup adds the supervisor's documented termination grace. This bounds time spent in custom rendering, not arbitrary allocations or filesystem latency in trusted Python.
