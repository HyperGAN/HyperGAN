...
DEFAULT = {
    ...
    "prior": {"kind": "particles", "args": {"num_particles": 20000, "z_dim": 4}},
    ...
    "objectives": [],
    "optimizer": {"lr": 0.0006, "d_lr_mult": 1.5, "prior_lr_mult": 10.0, "betas": [0.0, 0.999], "prior_betas": [0.0, 0.999]},
...
def resolve_config(raw):
    ...
    for path in paths:
        parts = path.split(".")
        if parts[0] not in {"batch", "latent", "generated", "candidate", "components"}:
