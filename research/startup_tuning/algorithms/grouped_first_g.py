...
def decide(probe):
    ...
    for bank in banks:
        ...
        if status == 'nonfinite':
            return _abstain(probe, 'model_unresolved', failed_check='nonfinite')
        if status:
    ...
    for bank in banks:
        ...
        if coefficients == 'nonfinite':
            return _abstain(probe, 'model_unresolved', failed_check='nonfinite', confirmation=ranked)
    ...
    separable = []
    for coefficients, exact in zip(fits, exacts):
        if not (coefficients['kA'] > 0 and coefficients['kB'] > 0):
            return _abstain(probe, 'curvature', coefficients=fits, confirmation=ranked)
        joint = _joint(coefficients)
        if joint is None or not joint[2] > 0:
            return _abstain(probe, 'hessian', coefficients=fits, confirmation=ranked)
        ...
        if exact == 'missing_gradient' or not (coefficients['aA'] < 0 and coefficients['aB'] < 0
            ...
            return _abstain(probe, 'slopes', coefficients=fits, confirmation=ranked)
        ...
        if any(not _unit_interval(value) for value in factors):
            return _abstain(probe, 'separable_factors', symmetric=factors[:2], exact=factors[2:], confirmation=ranked)
        separable.append(factors)
    ...
    if any(result == 'nonfinite' for result in adam_results.values()):
        return _abstain(probe, 'model_unresolved', failed_check='nonfinite', confirmation=ranked)
    ...
    hashes = _cost(probe)['bank_hashes']
    if any(value is None for value in hashes.values()) or hashes['fitting_0'] == hashes['monitor'] or hashes['fitting_1'] == hashes['monitor']:
        return _abstain(probe, 'bank_identity', confirmation=ranked)
