...
Periodic previews operate on copied EMA state on CPU. Inference bundles remain portable for CPU sampling with the supported components. These small observation operations preserve training RNG and selected device; they do not move training back to CPU. Custom components that require CUDA-only operators may need a separately supported inference/rendering implementation. Native callbacks still execute synchronously in the training process; supervised bounded callbacks belong to the internal replicated service, whose GPU port is pending.
...
Run against an installed wheel outside the checkout for integration evidence, using an absolute tests path and `python -I -m pytest ... --import-mode=importlib`. The CUDA suite includes native updates/recovery and a two-GPU NCCL diagnostic. Two GPUs on one host establish local communication; they do not establish full replicated GAN updates, coordinated GPU recovery, image quality or a real cluster.

The remaining GPU work is rank-owned devices and NCCL-aware reductions, CUDA accumulation/replay state, coordinated checkpoint staging, GPU worker supervision and public execution-profile routing. Full two-GPU D/G/prior/Adam/EMA parity and recovery must pass before that path is advertised as supported. Actual two-host testing follows with a concrete cloud allocation. See the [execution ledger](../reports/resurrection-status.md).
