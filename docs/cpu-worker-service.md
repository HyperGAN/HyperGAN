...
Existing [blocking CPU worker supervision](cpu-workers.md), numerical trainers and public single-process lifecycle are unchanged. The next integration must connect a replicated adapter, parent-only checkpoint publication and observer isolation to the shared controller before exposing distributed training commands.
