...
def _factor(value, version):
    return _positive(value) and value <= MAX_G_LR_FACTOR and (version >= 3 or value >= MIN_G_LR_FACTOR)


def _effective_rate(baseline, factor, player):
    if not _positive(baseline) or not _positive(baseline * factor):
        raise ValueError(f'Startup {player} baseline and effective learning rates must be finite and positive')
...
def generator_lr_override(config, factor):
    ...
    return {'schema_version': 1, 'scope': 'generator-main-group', 'factor': float(factor),
            'baseline_g_lr': base, 'effective_g_lr': _effective_rate(base, factor, 'generator'),
            'optimizer_config_sha256': optimizer_fingerprint(config)}


def optimizer_lr_override(config, g_factor, d_factor):
    """Schema three records positive G/D reductions without clipping a small rate.

    Historical schema one/two retain their original factor floor when read.
    The configured prior rate remains outside this override's authority.
    """
    for name, value in (('generator', g_factor), ('discriminator', d_factor)):
        if not _factor(value, 3):
            raise ValueError(f'Startup {name} learning-rate factor must be positive and at most 1')
    base = config['optimizer']['lr']
    baseline_d = base * config['optimizer']['d_lr_mult']
    return {'schema_version': 3, 'scope': 'generator-and-discriminator-main-groups',
            'factor': float(g_factor), 'd_factor': float(d_factor),
            'baseline_g_lr': base, 'effective_g_lr': _effective_rate(base, g_factor, 'generator'),
            'baseline_d_lr': baseline_d, 'effective_d_lr': _effective_rate(baseline_d, d_factor, 'discriminator'),
            'optimizer_config_sha256': optimizer_fingerprint(config)}
...
def checkpoint_base_lrs(trainer, metadata):
    ...
    Both preserve their 0.1 factor floor; schema three permits factors in (0, 1].
    """
    ...
    scope = {1: 'generator-main-group', 2: 'generator-and-discriminator-main-groups',
             3: 'generator-and-discriminator-main-groups'}
    ...
    if (not _factor(d_factor, version)
