...
The numerical identity records profile name, global batch, world size, local batch, accumulation, microbatch size and accumulation algorithm. Resume can increase the total step target only when both saved and requested `training.lr_floor` are `1.0` (constant learning rate), with all other numerical settings unchanged. Decreases and target changes for annealed runs are rejected. The manifest and new checkpoints record the extended target; existing checkpoint fingerprints remain unchanged, and resume without a configuration inherits the extension. Operational timeouts and checkpoint/stop policy can change independently. Checkpoints also validate actual runtime, implementation source, data identity and rank state; matching the profile alone does not prove compatibility.
