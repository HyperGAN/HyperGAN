...
def custom_generator(config, gan, net):
    ...
    net = tf.nn.relu(net)
    net = linear(net, 2560, scope="g_lin_proj3")
    net = tf.tanh(net)
    net = tf.reshape(net, [int(net.get_shape()[0]), 64, 80])
    gan.graph.g = net
    print("G___", net)
    return [net]
...
def train():
    ...
    if args.distribution == 'text':
        ...
        x = tf.reshape(x, [1, int(x.get_shape()[0]), int(x.get_shape()[1])])
        x = tf.tile(x, [64, 1, 1])
    ...
    with tf.device(args.device):
        ...
        for i in range(limit):
            ...
            if i % args.sample_every == 0 and i > 0:
                g, x_val = gan.sess.run([gan.graph.g[0], gan.graph.x], {gan.graph.z[0]:z_0})
                ...
                def output(val):
                    ...
                    return ox_val
                ox_val = output(x_val[0])
                print("X:"+str(i), x_val.shape)
                print("".join(ox_val))
                print("G:", g.shape)

                g = np.reshape(g, [g.shape[0], 64, 80]) 
