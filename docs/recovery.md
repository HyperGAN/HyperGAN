...
`train CONFIG --run-dir RUN_DIR` creates a new run when the directory is absent and resumes its latest complete checkpoint when it already exists. The supplied resolved configuration must match the saved run, apart from the constant-learning-rate target extension described below; comments and formatting do not affect that comparison. An unrelated directory, incomplete run without a full checkpoint, or invalid checkpoint fails without starting a fresh experiment in that directory. Use a new run directory for other changed training settings. Repeated `train` also rejects changed metrics configuration. To intentionally change observation-only settings, use `hypergan resume RUN_DIR --config CONFIG`; its numerical compatibility checks still apply.

`--steps` on `train` sets the total update target, not an additional step count. On an existing run, you can increase `training.steps` in the configuration or pass a larger `--steps` only when both saved and requested `training.lr_floor` are `1.0` (constant learning rate), with all other numerical settings unchanged. Decreases and target changes for annealed runs are rejected. `resume RUN_DIR --config CONFIG` accepts the same extension. An accepted extension is recorded in the run manifest and newly written checkpoints; existing checkpoint payloads and fingerprints remain unchanged. `hypergan resume RUN_DIR` then inherits the extended target without needing the original project or override.

`--stop-after-steps` limits only the current attempt. `--max-seconds 60` requests a cooperative wall-time stop at an update boundary; startup, a running update and final artifact writing can exceed that budget. It is not a process deadline. Changing architecture, resolution, labels, losses or other numerical settings requires a new experiment. Partial transfer initialization is not implemented.
...
Every resume, including repeating `train`, creates a new attempt with a monotonic index and unique identifier. Repeating a completed run without extending its target validates and restores it without additional training updates; it still records a new attempt and final artifacts. Existing attempt artifacts are never overwritten. `sample RUN_DIR` uses the bundle selected by the run manifest, and repeated sampling uses unique filenames. Explicit `--output` refuses an existing file. Older checkpoints remain available through `resume RUN_DIR --checkpoint PATH`; replaying one creates a new attempt without replacing earlier samples. Use `inspect` to select the checkpoint's recorded path.

The checkpoint includes generator/discriminator/auxiliary state, prior, EMA, Adam states and original learning rates, update counters, named RNG streams, global CPU Torch/Python/NumPy RNG states, and the declared data state. Resume validates the saved configuration and execution/data identity before continuing. `--config CONFIG` must remain numerically compatible, with the constant-learning-rate target extension allowed above; it does not replace saved model or optimizer state.
...
A rejected runtime names every field that differs, as dotted key paths with the saved and the current value (for example `cuda.name: saved "NVIDIA RTX A6000", current "NVIDIA GeForce RTX 4090"`), so the incompatibility is identifiable without diffing checkpoint metadata by hand. One difference is qualified rather than rejected: `cuda.uuid` and `cuda.visible_devices` record which physical card ran, not what it computes. Two identical GPUs in one machine enumerate in an unstable order unless `CUDA_DEVICE_ORDER` pins it, so a restart can restore onto the other card. When the difference is confined to those two fields — the model, capability, CUDA/cuDNN/Torch versions, dtype, world size and every determinism, TF32 and matmul setting all still match — the resume warns, naming both devices, and continues. The warning goes to stderr at resume time, into the run manifest as `warnings` and `resume_warnings`, and onto the `resume` event in the event log. Export `CUDA_DEVICE_ORDER=PCI_BUS_ID` and select the card with `CUDA_VISIBLE_DEVICES` to pin one physical device across restarts.
