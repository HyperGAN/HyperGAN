...
def generator(config, net):
    ...
    net = block_conv(net, activation, batch_size, 'identity', 'g_layers_init', output_channels=int(net.get_shape()[3]), filter=3)
    ...
    if batch_norm: 
        net = batch_norm(config['batch_size'], name='g_bn_first3_'+str(i))(net)
    net = config['generator.final_activation'](net)
    nets.append(net)
    return nets
