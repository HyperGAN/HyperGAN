...
Use `--preview-every N` for bounded periodic EMA previews. `hypergan events RUN_DIR` reads reconnectable event pages, and `hypergan checkpoint RUN_DIR` requests a save at the next complete update boundary. These commands share the [run observation contract](docs/observation.md); the optional [local browser and streaming API](docs/local-web.md) now plots file-backed metrics and exposes indexed samples. Install `.[web]`, run `hypergan project RUN_DIR --follow`, and use `hypergan serve RUN_DIR --open` in another terminal. The browser and external API clients share the same authenticated interface. Metric defaults are removable in [configuration](docs/configuration.md); `hypergan metrics RUN_DIR` and `hypergan contributions RUN_DIR` support headless agents.
