...
- The default group has 2–64 ranks; CPU fixtures qualify two ranks with one CPU thread each and a fixed Gloo process group. The same broker also supports [rank-owned CUDA/NCCL execution](replicated-cuda.md). Elastic membership and retries are not provided.
...
The [replicated run adapter](replicated-run-service.md) now connects these commands and parent checkpoint publication to the shared controller. It validates complete numerical boundaries in addition to the command protocol. The same broker supports [bounded observer delivery and isolated preview rendering](replicated-observation.md) and the [public execution commands](execution.md). Complete local numerical/recovery and fault evidence is recorded in the [execution ledger](../reports/resurrection-status.md); real multi-host qualification remains separate. The [blocking CPU worker supervisor](cpu-workers.md) remains available for finite callbacks.
