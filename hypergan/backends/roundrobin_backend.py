...
def train(device, gan, inputs, loaded_event, report_weights_queue, set_weights_queue, report_weights_event):
    gan.inputs = inputs
...
class RoundrobinBackend(Backend):
    ...
    def __init__(self, gan, cli, devices):
        ...
        for device in devices:
            ...
            p = mp.Process(target=train, args=(device, gan, inputs, loaded_event, report_weights_queue, set_weights_queue, report_weights_event))
            p.start()
    ...
    def step(self):
        ...
        for p1, p2 in zip(self.gan.parameters(), params):
            with torch.no_grad():
                p1.set_((p1+p2.to(p1.device))/2.0)
        del params
