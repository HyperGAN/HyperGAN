...
from hypergan.objective_program import _bound_scores, _generator_tail
...
def extra_generator_update(trainer):
    ...
    try:
        ...
        return dict(zip(('g_loss', 'g_adversarial', 'prior_loss'), values))
    finally:
        for parameter, flag in zip(fixed, flags):
            parameter.requires_grad_(flag)


def prepare(*, ratio=4, warmup_rounds=32, warmup_g_factor=1.):
    ...
    def context(trainer):
        ...
        proposal = {
            ...
            'extra_g_semantics': 'fresh real/latent draws; fixed D parameters/buffers and prior parameters/moments; G EMA per G update',
        ...
        def update(*args, **kwargs):
            ...
            for _ in range(ratio - 1 if warming else 0):
                result = extra_generator_update(trainer)
...
def main():
    ...
    report = run_probe(source, g_lr=opt['lr'], d_lr=opt['lr'] * opt['d_lr_mult'],
                       ...
                       prepare=prepare(ratio=args.ratio, warmup_rounds=args.warmup_rounds,
                                       warmup_g_factor=args.warmup_g_factor),
