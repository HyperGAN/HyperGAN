...
class AliNextFrameGAN(BaseGAN):
    ...
    def create(self):
        ...
        with tf.device(self.device):
            ...
            ue0 = UniformEncoder(self, config.z_distribution, output_shape=self.ops.shape(trash.sample))
            unrolled_c = C(z2, tf.zeros_like(ue0.sample), reuse=False)
            c_t_prev = C(z1, unrolled_c.sample).sample
            c = C(z, c_t_prev)

            self.c2 = unrolled_c.sample
            self.c1 = c_t_prev
            self.c = c.sample
            ...
            video_control = cz.reuse(self.c1)
            ...
            self.cz_next = cz.reuse(self.c1)

            c_up = cz.reuse(self.c)
            c_up1 = cz.reuse(self.c1)
            c_up2 = cz.reuse(self.c2)
            c_prev_hat = generator.reuse(tf.zeros_like(x_input), replace_controls={"z":c_up1})
            ...
            if config.train_gc:
                ...
                d = self.create_component(config.discriminator, name='d_fa', input=stacked, features=[features])
                l = self.create_loss(config.loss, d, x_input, generator.sample, len(stack))
                loss1 = l
                d_loss1 = l.d_loss
                g_loss1 = l.g_loss
...
class VideoFrameSampler(BaseSampler):
    def __init__(self, gan, samples_per_row=8):
        sess = gan.session
        self.c, self.c1, self.c2, self.z1, self.z2 = sess.run([gan.c, gan.c1, gan.c2, gan.z1, gan.z2])
    ...
    def _sample(self):
        ...
        self.c, self.z, next_frame = sess.run([gan.c, gan.cz_next, gan.video_sample], {gan.z1: self.z1, gan.z2: self.z2, gan.c1: self.c1, gan.c2: self.c2})
        self.z2 = self.z1
        self.z1 = self.z
        self.c2 = self.c1
        self.c1 = self.c
...
class TrainingVideoFrameSampler(BaseSampler):
    def __init__(self, gan, samples_per_row=8):
        ...
        self.x_input, self.last_frame_1, self.last_frame_2, self.z1, self.z2, self.c1, self.c2 = gan.session.run([gan.x_input, gan.inputs.x1, gan.inputs.x2, gan.z1, gan.z2, gan.c1, gan.c2])
    ...
    def _sample(self):
        ...
        c, c1, c2, c_prev_hat, x_hat, self.z, next_frame = sess.run([gan.c, gan.c1, gan.c2, gan.c_prev_hat, gan.x_hat, gan.cz_next, gan.video_sample], {gan.c1: self.c1, gan.c2: self.c2, gan.z1: self.z1, gan.z2: self.z2, gan.x_input:self.x_input, gan.last_frame_1:self.last_frame_1, gan.last_frame_2:self.last_frame_2})
