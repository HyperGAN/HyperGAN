...
The next milestone is a small, complete image-generation workflow using ParticleGAN MoG + b-cap and a pretrained discriminator from the start. The owner-selected starting experiment is the scratch SAGAN-style generator with frozen ResNet18 discriminator features, recorded at FID50k 12.5345 after 200k CIFAR-10 updates. The [next-version image plan](image-training-plan-2026-09-19.md) pins its source, distinguishes reproduction from external benchmark comparison, and defines the implementation gates. The long-term product is a curated collection of proven GAN applications with tested training and deployment profiles. Preserve historical experiments through Git archives rather than carrying them into the supported package.
...
Owner direction, 2026-09-19: GPU execution is the product default. This development machine has two local RTX A6000 GPUs authorized for qualification. Keep small CPU tests for correctness and dependency-free CI; advance native CUDA and actual two-GPU NCCL work alongside the remaining public distributed CLI gates. Local GPU use does not consume the reserved Modal allocation.

**Current execution order: qualify the pretrained-discriminator MoG/b-cap image recipe → provide image grids and the complete local user journey → reproduce and evaluate CIFAR-10 → qualify the actual image workload on two GPUs → publish comparable benchmark evidence → prove two-node training → finish deployment → release.** Historical preservation, foundation CI, numerical recovery, local GPU execution and public observation/profile routing are already implemented; do not repeat them. Competitive results and framework reliability both need evidence. Treat each numbered item as a milestone with one or more small PRs. Check it off only when its stated evidence is linked in [the execution ledger](resurrection-status.md).
...
- [x] **3. Land ParticleGAN inside a HyperGAN-owned loop.** CPU reference, configurable components, EMA and inference reload are implemented in PR #301, and complete two-process CPU updates are implemented in PR #308. The pinned runtime, source identities, numerical comparisons and current-run recovery are implemented; on 2026-09-20 the owner confirmed MIT distribution permission for the selected ParticleGAN image port, removing its source-permission hold. Further image qualification and release gates remain separate below. The original implementation scope was to pin the dependency/runtime and reproduce the tiny reference run. Define the next-version configuration, recipe and execution-profile schemas. Implement the first G/D/prior step, EMA, named RNG streams and run events through the new loop; add a small multi-process correctness fixture immediately. Declare a working version such as `2.0.0a1` after confirming release history rather than saving versioning until the end. **Done when:** the new CLI runs a bounded reference experiment, numerical comparisons pass, and every result records its source/config/runtime. This is the next version's foundation.
...
- [ ] **5. Qualify the first pretrained-discriminator image recipe.** Image-folder validation, data identity and complete native/replicated recovery are implemented. Follow [I1–I4 of the image plan](image-training-plan-2026-09-19.md): preserve factory-defined frozen parameters; port the selected scratch generator, pretrained discriminator and MoG/b-cap semantics; add real image grids; reproduce the recorded CIFAR-10 protocol with explicit FID evaluation. Preserve or validate changes to the source's encoder-only reconstruction and phase sampling. Pretrained weights and transfer initialization have explicit provenance separate from resume. **Done when:** the public workflow produces useful images, survives interruption/recovery, loads inference in a fresh process, and publishes source-matched quality/diversity/resource evidence. No broad architecture or seed-only sweep is part of this first port.

- [ ] **6. Qualify the real image workload on two GPUs.** [Local CUDA/NCCL numerical training and recovery](core-replicated-cuda-2026-09-19.md) and [public execution profiles](core-public-execution-2026-09-19.md) are implemented. Follow I5 of the image plan with the same image recipe at fixed global batch: verify phase draws, normalization, accumulation, exact gradient penalties and complete optimizer/EMA state. Exercise rank failure and coordinated recovery on the two authorized local GPUs. **Done when:** controlled comparisons and actual image learning pass with declared numerical tolerances, useful quality, and a throughput/memory report. This remains open until the image-workload evidence exists.

- [ ] **6a. Publish an externally comparable small benchmark result.** Follow I6 of the image plan: full-data unconditional CIFAR-10 first, then a proposed 10%-data comparison to test pretrained-feature value. Pin each external table's reference split, generated sample count, evaluator and checkpoint-selection policy; disclose pretraining data and compute. **Done when:** reproducible configurations, artifacts, samples and quality-versus-time results are public and a submission is accepted where eligible. A historical FID number or submission invitation is not verified leaderboard placement.
...
- [ ] **8. Complete the image and cluster user journey.** Public local train/resume profiles and the optional browser/API are implemented. Finish the image-specific experience during step 5: sensible qualified defaults, real PNG grids, fixed-latent progress, fresh sampling and a start-to-finish walkthrough. Later make submission/status/cancellation and artifact retrieval coherent with the selected cluster profile. Preserve `--no-server`, independent reconnectable observation and headless workers. **Done when:** first-time users can finish the supported workflow without maintainer intervention or manual Python edits, with clear recovery and resource information.
...
The image baseline is separate from the qualified synthetic numerical reference. The owner's 2026-09-19 selection is now the **SAGAN-style scratch generator plus pretrained ResNet18 discriminator, MoG16,384 and exact b-cap**, pinned by the [image plan](image-training-plan-2026-09-19.md). Its original 200k trajectory records FID50k12.5345. Preserve the actual source behavior, including GroupNorm, active attention, phase draws and encoder-only reconstruction; do not substitute vanilla DCGAN defaults. The earlier residual MoG1,024 candidate's FID50k19.483 at10k remains historical evidence in the [core recovery report](core-recovery-2026-09-18.md), not the next port target. The selected feature-branch image port is now authorized for HyperGAN MIT distribution, with no additional attribution requirement because the projects share authorship. Preserve source references for reproducibility and use public package primitives with complete HyperGAN recovery. The [execution report](image-training-execution-2026-09-20.md) records the implemented deterministic variant and its 40k allocation; public-port integration is tracked in the status ledger.
...
The initial audit found no standalone upstream license file or package license metadata despite its README's MIT declaration. The owner's 2026-09-20 MIT permission resolves the selected copied-port distribution gate without an additional attribution requirement. This does not relicense external pretrained weights, datasets or unrelated third-party code; release packaging remains a separate milestone. For sampling semantics, a deterministic generator drawing only from a finite atomic prior has at most one output per atom for fixed conditioning. Creative interpolation changes the sampling distribution; MoG/noisy sampling has different semantics. Record the prior kind and all noise/standardization state, and label interpolation as exploration rather than validated prior sampling.
...
The accepted [local web view plan](local-web-view-plan-2026-09-18.md) defines optional dependencies, local startup defaults, `--no-server`, standalone `serve`, offline assets and phased acceptance tests. Local serving, automatic CLI startup and the shared API/browser are now [implemented](metrics-implementation-2026-09-19.md); the image plan adds bounded PNG grids and image-specific acceptance. No UI callback should mutate training tensors directly. The UI uses the same run service through a loopback endpoint and per-session access control. A disconnected viewer must not stop a local or cluster job, and reopening it must recover progress from run artifacts and job state. Cluster access should initially use existing scheduler credentials and user-controlled connections. Building a hosted multi-tenant control plane is a separate later product, not a prerequisite for real cluster training.
