...
def train(config, inputs, args):
    ...
    for i in range(args.steps):
        ...
        #train on look ahead
        k = 10
        zt = gan.video_generator_last_z
        ct = gan.video_generator_last_c
        zs, cs = gan.session.run([zt, ct])
        zs = [zs]
        cs = [cs]
        for j in range(k):
            feed_dict = {}
            feed_dict.update(dict(zip([zt],zs)))
            feed_dict.update(dict(zip([ct],cs)))
            z_sample, c_sample = gan.session.run([gan.video_generator_last_zn, gan.video_generator_last_cn], feed_dict=feed_dict)
            zs = zs[1:] + [z_sample]
            cs = cs[1:] + [c_sample]

        gan.step({zt: zs, ct: cs})
